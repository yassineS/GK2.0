...
class BamToBWA(Tool):
    ...
    def cmd(self, (i, ), s):
        ...
        return (cmd_init + cmd_rg + cmd_main + cmd_out)#.format(**locals())

        #ipdb.set_trace()

        #return (cmd_init + cmd_main + cmd_out), \
        #       dict(i={input}),s=settings,**locals())

# VCF to Annovar

class VcfToAnno_in(Tool):
    name = "Convert VCF to Annovar"
    forward_input = True
    time_req = 12 * 60
    inputs = [inp(format='vcf', n='>0')]
    outputs = [out('out', format='anno_in')]
